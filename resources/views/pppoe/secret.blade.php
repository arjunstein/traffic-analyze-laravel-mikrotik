        ...
        <div class="page-inner py-5">
            <div class="d-flex align-items-left align-items-md-center flex-column flex-md-row">
                <div>
                    ...
                    <h5 class="text-white op-2 mb-2">Total Secret PPPoE : {{ $totalSecret }} </h5>
                </div>
            </div>
        </div>
    </div>
            ...
            <div class="card">
                ...
                <div class="table-responsive">
                    <table id="add-row" class="display table table-striped table-hover">
                        ...
                        <tbody>
                            @foreach ($secret as $no => $sec)
                                <tr>
                                    <div hidden>{{ $id = str_replace('*', '', $sec['.id']) }}</div>
                                    <td>{{ $no + 1 }} </td>
                                    <td>{{ $sec['name'] ?? '' }} </td>
                                    <td>{{ $sec['password'] ?? '' }} </td>
                                    <td>{{ $sec['service'] ?? '' }} </td>
                                    <td>{{ $sec['local-address'] ?? '' }} </td>
                                    <td>{{ $sec['remote-address'] ?? '' }} </td>
                                    <td>
                                    ...
                                    <td>
                                        <div class="form-button-action">
                                            <a href="{{ url('pppoe/secret/edit/' . $id) }}"
                                                class="btn btn-link btn-primary btn-lg" data-toggle="tooltip"
                                                data-original-title="Edit Task">
                                                <i class="fa fa-edit"></i>
                                            </a>
                                            <a href="{{ url('pppoe/secret/delete/' . $id) }}" type="button"
                                                data-toggle="tooltip" class="btn btn-link btn-danger"
                                                data-original-title="Remove"
                                                onclick="return confirm('Apakah anda yakin menghapus secret {{ $sec['name'] }} ?')">
